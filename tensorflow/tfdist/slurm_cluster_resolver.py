...
import hostlist as hostlist_mod
import os
import re
import subprocess
...
def expand_hostlist(hostlist):
  ...
  hosts = []
  hosts = hostlist_mod.expand_hostlist(hostlist)
  # try:
...
def _get_num_nvidia_gpus():
  ...
  """
  try:
    return len(os.environ['LUMI_VISIBLE_DEVICES'].split(','))
  except KeyError:
    pass  # Ignore and fallback to using nvidia-smi
  try:
    output = subprocess.check_output(['xnvidia-smi', '--list-gpus'],
                                     encoding='utf-8')
